...
(defn stop-channel
  [clients channel]
  ;; (nineteen/halt-skeletons (get-in clients [channel :every]))
    ...
    (if (httpkit/websocket? channel)
      (httpkit/on-receive 
       ...
       (fn [raw]
         ...
         ;;   (let [every (nineteen/provide-skeletons (send-skeletons channel) 50)]
...
(defn -main 
  ...
  (let [port 19991]
    ...
    ;; (nineteen/wake-skeletons)
    (println (format "nineteen started on %s" port))))
